DELIMITER //
CREATE PROCEDURE insert_or_update_location(w_name VARCHAR(255))
BEGIN
    ...
    ELSE
        ...
        SELECT 1 INTO name_exists FROM locations WHERE waterBody = w_name LIMIT 1;
    END IF;

    -- if name doesn't exist, insert
    If name_exists = 0 THEN
        SELECT IFNULL(MAX(id),-1) INTO highest_id FROM locations;
        INSERT INTO locations (id, waterBody) VALUES (highest_id + 1, w_name)
        ON DUPLICATE KEY UPDATE waterBody=waterBody;
    END IF;
END;
//
DELIMITER ;